...
export async function code(ctx: AgentContext): Promise<string> {
  ...
  const branchRes = await ctx.tools.execute("github.create_branch", { branch }, toolCtx(ctx, "code"));
  ...
  if (task) {
    task.status = "done";
    task.result = { branch, pr: prRes.output, impl: impl.text } as any;
    task.updatedAt = new Date().toISOString();
  }
  ctx.mission.filesChanged += (modRes.output as any)?.files ?? 1;
  ...
  return `Code changes committed on ${branch}. PR: ${(prRes.output as any)?.url ?? "n/a"}. ${impl.text}`;
}
