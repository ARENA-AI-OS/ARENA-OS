...
export function rankProviders(
  ...
  const kind: ModelTaskKind = (taskKind ?? "any") as ModelTaskKind;
  const override = strategy?.[kind];
  const base: ModelProvider[] = override ?? DEFAULT_STRATEGY[kind] ?? ["claude", "openai", "gemini"];
  const ranked: ModelProvider[] = [...base, "mock"];
